...
def load_data():
    try:
        # Čitamo podatke (ttl="0" osigurava da nema keširanja)
        df = conn.read(ttl="0")
        if df is None or df.empty:
            return pd.DataFrame()
        
        # Obezbeđujemo da su nazivi kolona čisti (bez razmaka)
        ...
        # Izbacujemo redove gde je Datum nevalidan
...
if menu == "Unos Podataka":
    ...
    if df.empty or len(df) < 2:
        ...
        # Kalkulacije razlika sa popunjavanjem praznina
        df['Delta_Dana'] = df['Datum'].diff().dt.total_seconds().fillna(0) / (24 * 3600)
        df['Potrosnja_B1'] = df['Brojilo_1_Stanje'].diff().fillna(0)
        df['Potrosnja_B2'] = df['Brojilo_2_Stanje'].diff().fillna(0)
        df['Proizvedeno_Delta'] = df['TP_Proizvedeno_kWh'].diff().fillna(0)
        
        df['Potrosnja_Ukupna'] = df.apply(
            lambda x: x['Potrosnja_B1'] if x['Aktivno_Brojilo'] == 'Brojilo 1' else x['Potrosnja_B2'], axis=1
        )
        
        # Dodatno čišćenje za Plotly (uklanjanje nula iz prvog reda jer kvare statistiku)
        ...
        df_clean['COP'] = (df_clean['Proizvedeno_Delta'] / df_clean['Potrosnja_Samo_TP']).fillna(0)
